...
output_dir ='Jags-YdichXnomSsubj-example';
...
for i=1:nChains
    initsList(i) = struct('theta', theta);
end
...
mcmcChain = cell2struct(mcmcChain, strcat(s_labels,'Theta'));
...
for i = 1:numel(params)
    subplot(numel(params),numel(params),numel(params)*(i-1)+i);
    mbe_plotPost(mcmcChain.(params{i}),'credMass',0.95,'xlab','\theta',...
        'PlotTitle',['\theta ',s_labels{i}] );
end
